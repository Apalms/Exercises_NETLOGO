...
to setup
 clear-all
  ...
  let max-deg max[count link-neighbors] of turtles ;;initial max-deg as the maximum degree of the graph
  set x [] ;;color-list
  let k 0
  while [k < max-deg + 1][ ;; we create a list with max-deg+1 colors
    set x lput random 140 x
    set k k + 1]
...
to activation
  ...
  while[any? turtles with [not decision?]][
    ...
    ask turtles with [not decision?][
      ifelse random-float 1.0 <= p.a
        [set mark? true]
        [set mark? false ]
      ]
    set max_steps max_steps + 1
    ask turtles with [not decision?  and mark?][
        set color one-of x
        set neiColorList ([color] of link-neighbors)
        show neiColorList

      foreach neiColorList[ f ->
           set  x remove f x
        ]
...
activation</go>
    ...
    <enumeratedValueSet variable="p.a">
      <value value="0.1"/>
      <value value="0.2"/>
      <value value="0.3"/>
      <value value="0.4"/>
